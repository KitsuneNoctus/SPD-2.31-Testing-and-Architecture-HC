...
def is_desired_state(desired_state):
    return desired_state == 'well-done' or desired_state == 'medium'

def is_proper_temperature(time, temperature, pressure):
    return get_cooking_progress(time, temperature, pressure) >= WELL_DONE or \
...
def is_well_done(time, temperature, pressure, desired_state):    
    return is_desired_state(desired_state) and  \
           is_proper_temperature(time, temperature, pressure)
